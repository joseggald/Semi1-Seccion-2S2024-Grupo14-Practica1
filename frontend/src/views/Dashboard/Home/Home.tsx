...
const Home: React.FC = () => {
  ...
  const handleToggleFavorite = async (song: Song) => {
    try {
      ...
      setSuccessMessage(`Song ${song.is_favorite ? 'removed from' : 'added to'} favorites successfully!`);
    } catch (error) {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-white mt-4">Browse all songs</p>
          ...
          {filteredSongs.map((song, index) => (
            <div key={song.id} className="grid grid-cols-12 gap-4 p-4 items-center text-white hover:bg-gray-800 transition">
              ...
              <div className="col-span-2 flex justify-center space-x-4">
                ...
                >
                  <img src={song.is_favorite ? favTrueIcon : favFalseIcon} alt="Favorite" className="w-6 h-6" />
                </button>
                <button
